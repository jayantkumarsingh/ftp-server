...
void server(int port)
{
  ...
  while(1){
    ...
    if(pid==0){
      ...
      if (0!=gethostname(welcome_message,sizeof(welcome_message)))
      {
        sprintf(welcome_message,"Host Name Not Available");
      }
      strcat (welcome_message," ");
      strcat (welcome_message,VERSION);
...
{
  sscanf(cmdstring,"%s %s %s %s",cmd->command,cmd->arg, cmd->subarg1, cmd->subarg2);
}
