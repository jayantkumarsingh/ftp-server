...
ssize_t sendfile(int out_fd, int in_fd, off_t * offset, size_t count, State *state )
{
    off_t orig;
    char buf[BUF_SIZE];
    size_t toRead, numRead, numSent, totSent, totalSize;
...
void response(Command *cmd, State *state)
{
  ...
  switch(lookup_cmd(cmd->command)){
    ...
    case SITE: ftp_site(cmd,state); break;
    case PORT: ftp_port(cmd,state); break;
    case NOOP:
      if(state->logged_in){
        state->message = "200 Nice to NOOP you!\n";
      }else{
        state->message = "530 NOOP Fail.\n";
      }
      write_state(state);
      break;
    default: 
      strncpy (msg,cmd->command,5);
      strcat (msg, " 500 unknown command\n");
      state->message = msg;
      write_state(state);
      break;
  }
...
void ftp_port(Command *cmd, State *state)
{
  ...
  if(state->logged_in){
    int ip[4];
    //char buff[255];
    char *response = "227 Entering ACTIVE Mode\n";
    Port *port = malloc(sizeof(Port));
    //gen_port(port);
    //getip(state->connection,ip);
    ...
    if ((state->sock_active=socket(AF_INET,SOCK_STREAM,0))<0)
    {
      printf ("failed to create data socket\n");
      return;
    }
    /* Address can be reused instantly after program exits */
    setsockopt(state->sock_active, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof reuse);

    remote_address.sin_addr.s_addr=inet_addr(ip_addr);
    remote_address.sin_port=htons(port->p1*256+port->p2);
    remote_address.sin_family=AF_INET;
    if ((connect(state->sock_active,(struct sockaddr *)&remote_address,sizeof(struct sockaddr_in)))!=0)
    {
      state->message = "425 something is wrong, can't start active connection \n";
    }
    else{
    ...
    if(state->logged_in){
      ...
      if(state->mode == SERVER){
        if(access(cmd->arg,R_OK)==0 && (fd = open(cmd->arg,O_RDONLY))){
          ...
          connection = accept_connection(state->sock_pasv);
          close(state->sock_pasv);
          if(sent_total = sendfile(connection, fd, &offset, stat_buf.st_size, state)){
        ...
        if(access(cmd->arg,R_OK)==0 && (fd = open(cmd->arg,O_RDONLY))){
          ...
          //connection = accept_connection(state->sock_pasv);
          //close(state->sock_pasv);
          if(sent_total = sendfile(state->sock_active, fd, &offset, stat_buf.st_size,state)){
...
{
  //printf("in quote site help\n");
  //printf ("Jayant: Argument: %s\n",cmd->arg);
  //printf ("Jayant: Sub Argument1: %s\n",cmd->subarg1);
  //printf ("Jayant: Sub Argument2: %s\n",cmd->subarg2);
  char msg[100];
  if (state->logged_in)
  {
    switch(lookup(cmd->arg, qscmdlist_str,sizeof(qscmdlist_str)/sizeof(char *)))
    {
      case HELP: ftp_qshelp(cmd, state); break;
      case MODULE: ftp_qsmodule(cmd,state); break;
      case CRC: ftp_qscrc(cmd,state); break;
      case CHMOD: printf("In CHMOD\n"); state->message="200 In chmod\n"; break;
      case COPY: printf("In COPY\n"); state->message="200 In copy\n"; break;
      default:
      strncpy (msg,cmd->arg,5);
      strcat (msg, " 500 unknown command\n");
      state->message = msg;//"500 Unknown command\n";
      write_state(state);
      break;
...
/*Handle quote site help*/
void ftp_qshelp(Command *cmd, State *state)
{
  char helpstr[200];
  if (state->logged_in)
  {

    sprintf(helpstr,"214 \n%s\n%s\n%s\n%s\n%s\n",qscmdlist_str[HELP],qscmdlist_str[MODULE],qscmdlist_str[CRC],qscmdlist_str[CHMOD],qscmdlist_str[COPY]);
    state->message=helpstr;
  }
  else
    state->message="550 no help\r\n";
   
  write_state(state);
}

/*Handle quote site module*/
void ftp_qsmodule(Command *cmd, State *state)
{
  char helpstr[200];
  if (state->logged_in)
  {

    sprintf(helpstr,"200 [<my Module>]\r\n");
    state->message=helpstr;
  }
  else
    state->message="550 no module\r\n";
   
  write_state(state);
}

/*Handle quote site module*/
void ftp_qscrc(Command *cmd, State *state)
{
  char helpstr[200];
  unsigned int crcVal=0;
  if (state->logged_in)
  {
    crcVal=getcrc (cmd->subarg1);
    if (crcVal==0)
    {
      state->message="450 Requested Action Not Performed\r\n";
    }
    else
    {
      sprintf(helpstr,"200 %s: %x\r\n",cmd->subarg1, crcVal);
      state->message=helpstr;
    }
...
unsigned int getcrc (unsigned char *fileName)
{
  ...
  while (!feof(fp))
  {
    unsigned int nCount= fread (buff,sizeof(char),BSIZE,fp);
    if (ferror(fp))
    {
      fclose(fp);
      return 0;
    }
    crc=xcrc32(buff,BSIZE,crc);
  }
  crc ^=0xFFFFFFFF;
  return crc;
}
