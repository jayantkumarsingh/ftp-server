...
typedef struct Command
{
  char command[18];
  char arg[BSIZE];
  unsigned char subarg1[BSIZE];
  unsigned char subarg2[BSIZE];
} Command;
...
/* quote site Commands enumeration */
typedef enum qscmdlist 
{ 
  HELP, MODULE, CRC, CHMOD, COPY
  
} qscmdlist;

/* String mappings for cmdlist */
static const char *qscmdlist_str[] = 
{
  "HELP", "MODULE", "CRC", "CHMOD", "COPY"
};
...
/* Welcome message */
//static char *welcome_message = "Welcome to Jayant's ftp";
static char welcome_message[100];
...
void ftp_qshelp(Command *, State *);
void ftp_qsmodule(Command *, State *);
void ftp_qscrc(Command *, State *);
/*void ftp_qschmod(Command *, State *);
void ftp_qscopy(Command *, State *);*/
void ftp_quit(State *);
...
void my_wait(int);
unsigned int getcrc (unsigned char *);
